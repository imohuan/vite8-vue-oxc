...
function getClaudeManagedDestinationPath(adapter, sourceRelativePath, input) {
  ...
  if (normalizedSourcePath.startsWith("rules/")) {
    return path.join(targetRoot, "rules", CLAUDE_ECC_NAMESPACE, normalizedSourcePath.slice("rules/".length));
  }
  ...
  if (normalizedSourcePath.startsWith("skills/")) {
    return path.join(targetRoot, "skills", CLAUDE_ECC_NAMESPACE, normalizedSourcePath.slice("skills/".length));
  }
    ...
    return modules.flatMap((module) => {
      ...
      return paths
        ...
        .map((sourceRelativePath) => {
          const managedDestinationPath = getClaudeManagedDestinationPath(adapter, sourceRelativePath, planningInput);

          if (managedDestinationPath) {
            return createRemappedOperation(adapter, module.id, sourceRelativePath, managedDestinationPath, {
