...
function discussionNeedsMaintainerTouch(discussion) {
  ...
  const comments = discussion.comments && Array.isArray(discussion.comments.nodes) ? discussion.comments.nodes : [];
...
function discussionNeedsAcceptedAnswer(discussion) {
  return Boolean(discussion && discussion.category && discussion.category.isAnswerable && !discussion.answer);
}
