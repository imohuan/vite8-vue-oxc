...
function pushFileEvent(events, value, action, diffPreview, patchPreview) {
  ...
  const normalizedDiffPreview = typeof diffPreview === "string" && diffPreview.trim() ? diffPreview.trim() : undefined;
...
function buildActivityRow(input, env = process.env) {
  ...
  const filePaths =
    fileEvents.length > 0 ? [...new Set(fileEvents.map((event) => event.path))] : extractFilePaths(toolInput);
...
function run(rawInput) {
  try {
    ...
    if (row) {
      appendFile(path.join(getClaudeDir(), "metrics", METRICS_FILE_NAME), `${JSON.stringify(row)}\n`);
    }
  } catch {
