...
function probeCommandServer(serverName, config) {
  return new Promise((resolve) => {
    ...
    const isPathLike = commandIsString && (path.isAbsolute(command) || command.includes("/") || command.includes("\\"));
    const candidates =
      ...
      timer = setTimeout(() => {
        ...
        if (child.exitCode !== null || child.signalCode !== null) {
          attemptFinish({
            ...
            reason:
              stderr.trim() || `process exited before handshake (${child.signalCode || child.exitCode || "unknown"})`,
          });
          return;
        }
