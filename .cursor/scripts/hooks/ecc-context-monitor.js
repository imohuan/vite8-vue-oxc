...
function evaluateConditions(bridge, options = {}) {
    ...
    if (cost > COST_CRITICAL_USD) {
      warnings.push({
        ...
        message:
          ...
          "Stop and inform the user about high cost before continuing.",
      });
  ...
  if (fileCount > FILES_WARNING_COUNT) {
    warnings.push({
      ...
      type: "scope",
      message:
        `SCOPE WARNING: ${fileCount} files modified this session. ` + "Consider whether changes are too scattered.",
    });
  }
